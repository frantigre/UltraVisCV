...
void getDataset(std::vector<std::vector<cv::Mat>>& data, std::vector<GroundTruth>& labels, std::vector<std::string>& sampleNames, std::string root) {
...
void getSequence (std::string path, std::vector<cv::Mat>& data, GroundTruth& labels) {
    ...
    if(!file)
        throw std::runtime_error("nodata GetLabel");
